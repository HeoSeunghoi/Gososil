...
function ChatTemplate() {
	...
	const getChatLog = () => {
		...
		// 9주차 실습 구현
		axios.get("http://localhost:8001/get").then(response => {
         if(response.data.length > chat_len){
            setChatLen(response.data.length);
	...
	const onSearchKeySubmit = (keyword) => {
		// 9주차 실습 구현
		chats.map((chat)=>{
			if (chat.message.search(keyword) != -1){
				chat.searched = true;
			}
			else chat.searched = false;
		})
	}
