...
function TodoTemplate() {
  ...
  const onRemoveDoneTodo = (id) => {
    // 7주차 실습 구현
    const deletedTodo = doneTodos.find((doneTodos) => doneTodos.id === id);
    setDeletedTodos([...deletedTodos, deletedTodo]);
    setDoneTodos(doneTodos.filter((doneTodos) => doneTodos.id !== id));
  };
  ...
  const onToggleDoneToDo = (id) => {
    // 7주차 실습 구현
    const Todo = doneTodos.find((doneTodos) => doneTodos.id === id);
    Todo.checked = false;
    setTodos([...todos,Todo]);
    setDoneTodos(doneTodos.filter((doneTodos) => doneTodos.id !== id));
  };
